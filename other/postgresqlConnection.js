...
await client.query('CREATE TABLE accounts (pid int, mii_data varchar(256), mii_hash2 varchar(256), nnid varchar(16), screen_name varchar(16), gender varchar(1), birth_date varchar(10), create_date varchar(19), email varchar(255), country varchar(10), utc_offset int, language varchar(10), mii_url varchar(255), tz_name varchar(255), update_time varchar(255), region int, serviceToken varchar(255), password varchar(255));', (err) => {
	if (!err) {
		console.log(logger.Postgresql("Account table made"))
	}
	})
await client.query('CREATE TABLE last_accessed (id varchar(255), rnid varchar(16))', (err) => {
	if (!err) {

        console.log(logger.Postgresql("OAuth table made"))
